...
// let a:string = returnNumber(); // 컴파일에러 발생 
...
/*
    3. Optional Parameter
     ...
     - 변수명?:type
     - Optional Parameter는 type | undefined와 동일하다.
*/
function fnOptional(a:number, b?:number) /*: number*/{
    ...
    /*
        ...
        조건문을 통해 타입을 좁히는(Narrowing) 작업이 필요하다.
    */
    if(b != undefined){
        return b;
    }
...
/*
    7. 타입 내로잉(Narrowing)
     - union type처럼 변수가 여러타입을 가지고 있을 때, 이를 사용하려고 하는
       "한가지 타입"으로 좁히는 문법
     - typeof, instanceof, in 연산자
*/
function typeNarrowing(strOrNumber:string|number):void{
    if(typeof strOrNumber === 'number'){
        console.log(strOrNumber.toFixed(2)); // 2자리 디지털 number로 고정
    }
    else{
        console.log(strOrNumber.toUpperCase()); // 대문자로 변환
    }
}
typeNarrowing(1000);
typeNarrowing("mkm");
...
/*
    8. Type Assertion (컴파일러에 특정 타입임을 강제로 알리는 문법)
...
function typeAssertion(strOrNumber:string|number):string{
    // return <string>strOrNumber.toUpperCase(); // 구버전
...
/*
    9. never type (값이 절대로 반환되지 않는 함수의 타입)
...
function typeNarrowing2(sOrn:etc):void{
    if(typeof sOrn == 'number'){
        ...
        console.log(sOrn); // never 타입
        const check:never = sOrn;
        // 현재 sOrn에 새로운 타입이 추가되는 경우 위 코드에서 컴파일 에러를 발생 시킴으로써
        // 모든 타입이 철저히 검사되고 있는 지 확인 할 수 있다.

        // 타입체크용 메서드
        // 컴파일 단계의 타입체크기능을 무효화하여 check에 string,number와 같은 다른 값이 전달 된 경우,
        // 런타임시 에러가 발생할 수 있도록 처리하는 함수
...
/*
    10. 함수 타입 표현식 / 함수 시그니처 (함수의 매개변수와 반환값 타입을 변수에 지정하는 문법)
     - 함수의 시그니처를 타입으로 정의하는 방법
     - 함수의 시그니처란?
        => 함수가 가지는 매개변수의 숫자, 개수와 자료형, 반환형을 합친 것.    
*/
...
/*
    11. type Aliases ((복잡한) 타입에 이름을 붙여 재사용성을 높이는 문법)
     - 복잡한 타입을 별칭으로 관리하기 위한 문법
     - 타입의 가독성과 재사용성을 크게 늘려주는 문법
     - 대문자로 시작하는 것이 관례
*/
type PrinFn = (s:string) => void;
type HelloFn = (callback : PrinFn) => void;
