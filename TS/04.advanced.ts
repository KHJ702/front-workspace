...
// 1) 제네릭과 타입추론
function returnGeneric<T>(value : T) : T{ // <T>는 타입추론으로 인해 string으로 지정됨.
    return value; 
}
...
function config<T extends Options = {theme:"dark"}>(option?:T)
{
    // return option ? option : {theme : "dark"} as T;
    // return option || {theme:"dark"} as T; 사용지양X
    // falsy? 값이 비어있으면 false

    // 널병합연산자 ??
    // - a ?? b -> a가 null 또는 undefiend인 경우 B 반환, 아닌 경우 A
...
function getValue<P extends Person2, K extends keyof Person2>(person:P, key:K): P[K] {
    return person[key];
}
const person:Person2 = {name:'mkm' , age : 18, location : 'seoul'};
// const value = getValue<Person2, "age">(person,"age"); 
// 원래는 이렇게 써야하나, 자동으로 추론할 수 있게 아래처럼 작성
const value = getValue(person, "location");
...
/*
    ...
    1) Partial<T>
     ...
     - 모든 속성을 필수, 읽기전용으로 만들어주는 Required<T>, Readonly<T>도 존재한다.
*/
interface User {
    id : number;
    name : string;
...
//const user:UpdateUser = {id:1, name:'aaa'}; // 수정할 데이터 중 email이 없는 경우 에러
...
/*
    Utility Type 조합
     ...
     3. 1, 2번을 병합
      ...
      -> {name? : string, email? : string, name : string}
      // 같은 속성이 겹치는 경우 더 구체적인(명확한) 속성으로 우선시 되어 병합.
*/
