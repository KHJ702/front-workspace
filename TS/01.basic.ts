/*
    1. 타입 주석
     ...
     - 타입주석을 통해 정적 타입 검사가 가능해지며,
       잘못된 타입의 값이 들어갈 경우 컴파일 오류가 발생.
*/
let num:number;
num = 12345;
//num = '12345';
//num = num * number('1'); // 타입스크립트는 암묵적 형변환 지원X
...
/*
  2. 타입 추론
   ...
   - 단, 복잡한 함수의 매개변수나 반환형, api함수와 같이 명확한 타입정의가 필요한 곳에는 반드시 작성한다.
*/
let userName = 'kh 타이거';
let age = 5;
...
let numArr : number[];
numArr = [1,2,3];
console.log(numArr[2], numArr[3]); // undefined, 에러는 발생하지 않는다.
numArr = [1,2,3,4,5];

/*
  튜플
   - 튜플은 배열의 특수한 형태로, "길이"와 "타입"이 고정된 배열이다.
   - 일반배열은 언제든 요소의 수가 변할 수 있지만, 튜플은 항상 고정된 구조를 가짐
*/
let numTuple:[number,number,number];
// numTuple = [1,2]; // X
numTuple = [1,2,3];
// console.log(numTuple[2], numTuple[3]); 
console.log(numTuple[2]); // 잘못된 인덱스 접근 시 에러 발생
...
/*
  4. readonly
   ...
   - 이 경우 튜플에 대한 논리적인 오류가 발생할 수 있느넫, 이를 방지할 수 있는게 readonly.
*/
numTuple.push(4);
...
// readonlyObject.name = 'mkm'; // readonly -> 할당불가

/* 5. any Type
    - 어떤 값이든 저장할 수 있는 타입.
    - 타입스크립트의 정적 타입 검사를 우회하기위해 설계되었다.
    - javascript코드와의 호환성 유지 및 마이그레이션을 돕기 위한 타입이고, 무분별하게 사용시
...
// 적절한 타입으로의 narrowing 필요.
// str = unknown; // X
// num2 = unknown; // X

/*
  7. union type
   - 여러 타입들을 하나로 묶어서 관리하는 타입
   - type1 | type2 | type3
*/
