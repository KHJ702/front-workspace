...
export default function Menuinput() {
    ...
    /*
        #2. useMutation
         ...
         - 요청을 보낸 후 요청상태(loading, error, success)를 관리할 수 있으며,
           요청 성공 시 캐시데이터를 무효화(invalidate)시켜 관련 데이터를 refetch한다.
    [사용법]
    const queryClient = uesQueryClient(); // 전역 캐시관리 객체. (쿼리무효화에 사용한다.)
    ...
    */
    ...
    const mutation = useMutation({
        ...
        onSuccess : (res) => {
            // 등록 요청 성공시
            queryClient.invalidateQueries({queryKey:['menus']}); // 메뉴목록데이터 캐시 무효화.

            // 리디렉션(서버에서 location을 지정해 주었다면 해당 경로로, 아니면 목록으로 이동)
